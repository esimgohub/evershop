...
async function deleteCustomerData(uuid, connection) {
  await update('customer').given({ email: '', external_id: '' }).where('uuid', '=', uuid).execute(connection);
}
/**
 ...
 */
async function deleteCustomerAccount(uuid, context) {
...
module.exports = async (uuid, context) => {
  ...
  const customer = await hookable(deleteCustomerAccount, context)(uuid, context);
  return customer;
};
